...
# ✅ Начинаем обновление
echo "📦 Обновляем официальный образ n8n..."
docker pull n8nio/n8n:latest || { echo "❌ Не удалось скачать образ"; exit 1; }

echo "🧹 Удаляем старый базовый образ (для пересборки без кэша)..."
...
echo "🔨 Пересобираем Dockerfile (без кэша)..."
docker-compose build --no-cache || { echo "❌ Ошибка при сборке Dockerfile"; exit 1; }
